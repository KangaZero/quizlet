<script lang="ts">
	// Props
	export let stars: number = 11; // Default star count
	export let repo: string = 'KangaZero/quizlet.io'; // Default repository

	// Compose the GitHub URLs
	...
	// Function to star the repository
	function starRepository(e: Event) {
	...
	<div class="flex items-center">
		<svg class="size-4" viewBox="0 0 438.549 438.549">
			<path
				...
				fill="#fff"
			></path></svg
		><span class="ml-1 p-1 text-white lg:inline">Star on GitHub</span>
	</div>
	<div class="ml-2 flex items-center gap-1 text-sm md:flex" on:click={starRepository}>
		<svg
		...
		<span
			class="font-display inline-block font-medium tracking-wider text-black tabular-nums dark:text-white"
			>{stars}</span
		>
	</div>
</button>
